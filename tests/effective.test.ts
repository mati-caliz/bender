...
describe("effectiveHeadersFor", () => {
    ...
    const state = stateWith({
      profiles: [
        profileWith({
          id: "a",
          name: "A",
          requestHeaders: [createHeaderEntry({ name: "x-token", value: "viejo" })],
        }),
        profileWith({
          id: "b",
          name: "B",
          requestHeaders: [createHeaderEntry({ name: "X-Token", value: "nuevo" })],
        }),
      ],
    });
  ...
  it("separa request de response", () => {
    ...
    expect(effectiveHeadersFor(state, URL).map((header) => header.direction)).toEqual([
      "request",
      "response",
    ]);
  });
  ...
  it("el user agent pisa el que puso un perfil a mano", () => {
    const state = stateWith({
      profiles: [
        profileWith({ requestHeaders: [createHeaderEntry({ name: "user-agent", value: "a mano" })] }),
      ],
