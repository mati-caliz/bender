...
const DomainInput = ({ label, hint, domains, onChange, suggestion }: DomainInputProps): ReactElement => {
  ...
  return (
    <div className="field">
      ...
      <div className="row wrap domain-input">
        ...
        {hasText(suggestion) && !domains.includes(suggestion) ? (
          <Button
            small
            variant="ghost"
            icon="plus"
        ...
        <div className="row wrap">
          {domains.map((domain) => (
            <Chip
              ...
              label={domain}
              onRemove={() => {
                onChange(domains.filter((candidate) => candidate !== domain));
              }}
            />
          ))}
        </div>
...
const RequestMethodsField = ({ scope, onChange }: ScopeFieldProps): ReactElement => {
  const toggleRequestMethod = (requestMethod: RequestMethod): void => {
  ...
  return (
    <div className="field">
        ...
        {ALL_REQUEST_METHODS.map((requestMethod) => (
          <label key={requestMethod} className="checkbox">
            <input
              ...
              onChange={() => {
                toggleRequestMethod(requestMethod);
              }}
            />
      ...
      {scope.requestMethods.length > 0 ? null : (
        <span className="field-hint">Vacio = todos los metodos.</span>
      )}
    </div>
  );
};

const ResourceTypesField = ({ scope, onChange }: ScopeFieldProps): ReactElement => {
  const [showResourceTypes, setShowResourceTypes] = useState(scope.resourceTypes.length > 0);

  const toggleResourceType = (resourceType: ResourceType): void => {
  ...
  return (
    <div className="field">
      <button
        ...
        onClick={() => {
          setShowResourceTypes((current) => !current);
        }}
      >
        <Icon name={showResourceTypes ? "chevron-down" : "chevron-right"} size={12} />
        Tipos de request
        {scope.resourceTypes.length > 0 ? ` (${scope.resourceTypes.length})` : " (todos)"}
      </button>
      {showResourceTypes ? (
...
export const ScopeEditor = ({ scope, onChange, currentHostname }: ScopeEditorProps): ReactElement => {
  const handleUrlFilterKey = (event: KeyboardEvent<HTMLDivElement>): void => {
    ...
    <div className="card-body" style={{ padding: 0, gap: 12 }}>
      <label className="row" style={{ cursor: "pointer" }}>
        <Switch
          ...
          onChange={(checked) => {
            onChange({ ...scope, activeTabOnly: checked });
          }}
          small
        />
        <div>
      ...
      <DomainInput
        ...
        onChange={(domains) => {
          onChange({ ...scope, includeDomains: domains });
        }}
      ...
      <DomainInput
        ...
        onChange={(domains) => {
          onChange({ ...scope, excludeDomains: domains });
        }}
      />
      ...
      <DomainInput
        ...
        onChange={(domains) => {
          onChange({ ...scope, initiatorDomains: domains });
        }}
        suggestion={currentHostname}
      />
      ...
      <DomainInput
        ...
        onChange={(domains) => {
          onChange({ ...scope, excludedInitiatorDomains: domains });
        }}
      />

      <RequestMethodsField scope={scope} onChange={onChange} />
      ...
      <ResourceTypesField scope={scope} onChange={onChange} />
    </div>
  );
};
