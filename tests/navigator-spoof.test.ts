...
const stateWith = (
  userAgent: Partial<UserAgentConfig>,
  overrides: Partial<ToolkitState> = {},
): ToolkitState => ({
  ...createDefaultState(),
...
describe("navigatorSpoofRegistration", () => {
  ...
  it("traduce los dominios del alcance a match patterns", () => {
    const registration = navigatorSpoofRegistration(
      stateWith({
        scope: scopeWith({
          includeDomains: ["Example.com", "otro.com"],
...
describe("navigatorSpoofDiagnostics", () => {
  it("no avisa nada con un alcance por dominio", () => {
    expect(
      navigatorSpoofDiagnostics(userAgentWith({ scope: scopeWith({ includeDomains: ["example.com"] }) })),
    ).toEqual([]);
  });
  ...
  it("no avisa nada con el spoof apagado", () => {
    expect(
      navigatorSpoofDiagnostics(
        userAgentWith({ spoofNavigator: false, scope: scopeWith({ activeTabOnly: true }) }),
      ),
    ).toEqual([]);
  });
});
