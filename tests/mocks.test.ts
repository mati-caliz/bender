...
describe("findMatchingMock", () => {
  ...
  it("filtra por el dominio que origina la request", () => {
    const mocks = [
      mockWith({ includeDomains: ["example.com"], initiatorDomains: ["app.local"] }, "desde-app"),
    ];
    expect(findMatchingMock(mocks, requestFor("https://example.com/api", "GET", "app.local"))?.id).toBe(
      "desde-app",
    );
